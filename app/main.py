...
async def main():
    ...
    db=Database(settings); await db.create_tables()
    bot=Bot(settings.bot_token,default=DefaultBotProperties(parse_mode=ParseMode.HTML))
    dp=Dispatcher()
    dp.message.middleware(RateLimitMiddleware()); dp.callback_query.middleware(RateLimitMiddleware())
    dep=DependenciesMiddleware(db,settings); dp.message.middleware(dep); dp.callback_query.middleware(dep)
    dp.include_router(admin_router); dp.include_router(extra_router); dp.include_router(user_router)
