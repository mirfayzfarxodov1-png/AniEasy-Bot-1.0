...
def main_menu(admin=False, webapp_url=None):
    rows = [
        ...
        [('🕘 Tarix', 'history'), ('🏷 Janr bo‘yicha', 'genres')],
        [('🔔 Yangiliklar obunasi', 'notify'), ('🌟 Premium', 'premium')],
        [('📌 Saqlangan', 'favorites'), ('🎯 Tavsiyalar', 'recommend')],
        [('📊 Bot statistikasi', 'bot_stats'), ('📝 So‘rov yuborish', 'request')],
...
def admin_menu():
    ...
    return ReplyKeyboardMarkup(
        ...
        resize_keyboard=True,
        is_persistent=True,
        input_field_placeholder='Admin funksiyasini tanlang…',
    )
...
def anime_actions(aid, favorite=True):
    return ikb([
        [('📺 Qismlar', f'episodes:{aid}:0'), ('⭐ Sevimli' if favorite else '☆ Sevimlidan chiqarish', f'fav:{aid}')],
        [('⬅️ Orqaga', 'anime'), ('🏠 Bosh menyu', 'home')],
    ])
